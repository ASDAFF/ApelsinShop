...
if ((isset($_POST['propertyId']) && isset($_POST['groupId']))) {
    $groupID = $_POST['groupId'];
    $propertyId = $_POST['propertyId'];
    $value = $_POST['value'];
    ShopPropertiesFilterSerchArray::unsetGroupFilterValue($groupID, $propertyId, $value);
    $countValues = ShopPropertiesFilterSerchArray::countPropertiesValue($groupID, $propertyId);

    $del = ShopGroupPropertyValue::getArrayForDeleteBlockProperty($groupID);
    if ($countValues < 1) {
        $html = '<script type="text/javascript">';
        $html .= '$("#PrValStr_' . $propertyId . '").remove();';
        if (count($del) < 1) {
