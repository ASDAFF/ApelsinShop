 ...
 * @author olga
 */
class AP_ChangeProductShop extends AdminPanel_ComponentPanelUI_Element {
    ...
    private function generateForm() {
        ...
        $this->html .= '<form class="AP_Form" name="AP_Form" action="'.$this->urlHelper->getThisPage().'" method="post" autocomplete="off" accept-charset="UTF-8">';
            $this->html .= '<fieldset>';
            $this->html .= '<legend>Найти товар</legend>';
    ...
    private function getSelectData() {
        $res = [];
        $result = ['0'=>'', 'itemName'=>'Найти по названию товара', 'id'=>'Найти по ID товара'];
    ...
    private function generationListItem() {
        ...
        if (count($this->data) > 0 && $this->data != null) {
            ...
            foreach ($this->data as $item) {
                ...
                $groupName = $this->getNameGroup($item['group']);
                $text = $this->getShown($item['shown']); 
                $params_edit[5] = $this->elementAliasID;
                $this->html .= '<div class="wrapperItemsShopItems">';
                    $this->html .= '<div class="blockGorizontConteinerShopItems">';
                        $this->html .= '<div class="leftBlockGorizontShopItems"><span class="itemsName">'.$item['itemName'].'</span> ('.$item['id'].')</div>';
                        $this->html .= '<div class="rightBlockGorizontShopItems '.$text['class'].'" id="actionShopImage'.$i.'" title="'.$text['text'].'" onclick="setShowHideItems(\''.$i.'\', \''.$item['id'].'\');">';
                            $this->html .= $text['text'];
                        $this->html .= '</div>';
                    $this->html .= '</div>';
                    $this->html .= '<div class="blockGorizontConteinerShopItems">';
                        $this->html .= '<div class="leftBlockGorizontShopItems ">группа - '.$groupName['groupName'].' ('.$item['group'].')</div>';
    ...
    private function getQuery() {
        $query = "SELECT * FROM `ShopItems` WHERE `".$this->type."` LIKE '%".$this->search[0]."%' ";
    ...
    protected function generateEditUI() {
        $changeProductShopEdit = new AP_ChangeProductShopEdit($this->URL_PARAMS[5]);
        echo $changeProductShopEdit->getHtml();
    }
    ...
    private function getShown($shown) {
        if ($shown == '0') {
            $text['text'] = 'Не показывать';
            $text['class'] = 'hideShopItems';
        } else {
            $text['text'] = 'Показывать';
