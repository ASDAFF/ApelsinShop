...
class TextGenerator {
    ...
    // Гласыная буква
    private static function isPunctuationMark($letter) {
        return in_array($letter, self::$punctuationMark);
    }

    // Гласыная буква
    private static function isVowels($letter) {
        return in_array($letter, self::$vowels);
    }

    // Согласная буква
    private static function isConsonants($letter) {
        return in_array($letter, self::$consonants);
    }

    // специальная буква
    private static function isSpecialСharacters($letter) {
        return in_array($letter, self::$specialСharacters);
    }

    // маска буквы
    private static function letterMask($letter) {
        if (self::isVowels($letter)) {
            return 1;
        }
        if (self::isConsonants($letter)) {
            return 2;
        }
        if (self::isSpecialСharacters($letter)) {
            return 3;
        }
        if (self::isPunctuationMark($letter)) {
            return 4;
        }
        return 0;
    }
